...
ثبت‌نام عمومی فقط داوطلب می‌سازد. staff از seed یا `POST /api/v1/auth/external` ساخته می‌شود.
...
- وب‌هوک `POST /api/v1/webhooks/events` با `X-Internal-Token` (دروازه) و `token` / `X-Mission-Token` (راز ماموریت) رویدادهایی مثل `user.invited` را ثبت می‌کند. داوطلب نمی‌تواند ماموریت را دستی تمام کند؛ «بررسی تأیید» سرویس داخلی یا وب‌سرویس تعریف‌شده در پنل بهره‌بردار را صدا می‌زند.
