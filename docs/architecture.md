...
تست واحد `internal/usecase/taskuc/reserve_test.go` این تضمین را با ۱۲ درخواست همزمان روی ظرفیت ۳ بررسی می‌کند. اگر قفل Redis در مهلت گرفته نشود درخواست با `busy` رد می‌شود؛ دیگر unlock خالی برنمی‌گردد.

لیست تسک‌های واجد شرایط در SQL فیلتر و صفحه‌بندی می‌شود (مهارت، حداقل امتیاز، رشته، و عدم رزرو قبلی) تا با ۱۰۰۰ داوطلب صفحه ناقص یا `total` غلط ندهد.

به‌روزرسانی ساعات و میانگین امتیاز اتمی است (`UPDATE ... completed_tasks + 1`) تا دو تسک همزمان امتیاز را گم نکنند. لغو رزرو هم در یک تراکنش ظرفیت را برمی‌گرداند.
